...
import { hierarchyTrees, seatTriangles } from "./hierarchy";
...
import { applySpacing, spacingFor } from "./spacing";
...
export const layoutDiscreteSearch = (
  ...
  const spacing = spacingFor(graph, arranged, params, { trees });
  const centres: Placement = applySpacing(spacing, arranged);
  // ...but a tree travels whole. Its members asked the pass for room from their
  // own boxes and were all answered on the root's line, so they follow the root
  // rather than the lines they happen to sit on -- which is what keeps the fan
  // the shape the tree pass gave it instead of stretching it unevenly.
  for (const tree of trees) {
    const before = arranged.get(tree.rootId);
    const after = centres.get(tree.rootId);
    if (before === undefined || after === undefined) continue;
    for (const id of tree.offsets.keys()) {
      const was = arranged.get(id);
      if (was !== undefined)
        centres.set(id, {
          x: was.x + (after.x - before.x),
          y: was.y + (after.y - before.y),
        });
    }
  }

  // the triangles a tree seated ride on their subclasses, so where they end up
  // is only known once the members have been placed, refined and spaced. Taken
  // from the spaced centres rather than carried through the transform: a seat is
  // one separation above its subclass, not a proportion of the row it sits in.
  const pinned = seatTriangles(graph, trees, centres, params);
